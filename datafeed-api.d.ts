...
export interface DatafeedConfiguration {
	 ...
	 *
	 * @example `["USD", "EUR", "GBP"]`
	 * @example `[{ id: "USD", code: "USD", description: "$" }, { id: "EUR", code: "EUR", description: "â‚¬" }]`
	 */
...
export interface LibrarySymbolInfo {
	 ...
	 *
	 * - `ohlcv` â€” the symbol supports open, high, low, close prices and has volume.
	 * - `ohlc` â€” the symbol supports open, high, low, close, prices but doesn't have volume.
	 * - `c` â€” the symbol supports only close price. This makes the chart show the symbol data using only line-based styles.
	 * @default 'ohlcv'
	 */
 ...
 * ---------|----------|---------
 * Ticks | `xT` | `1T` â€” one tick
 * Seconds | `xS` | `1S` â€” one second
 * Minutes | `x` | `1` â€” one minute
 * Hours | `x` minutes | `60` â€” one hour
 * Days | `xD` | `1D` â€” one day
 * Weeks | `xW` | `1W` â€” one week
 * Months | `xM` | `1M` â€” one month
 * Years | `xM` months | `12M` â€” one year
 *
